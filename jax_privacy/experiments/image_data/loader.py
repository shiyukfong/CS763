...
class DataLoader(metaclass=abc.ABCMeta):
  ...
  def _apply_class_balanced_subset(
      self,
      ds: tf.data.Dataset,
  ) -> tf.data.Dataset:
    num_per_class = self.config.class_balanced_num_per_class
    if num_per_class is None:
      return ds
    if num_per_class <= 0:
      raise ValueError('class_balanced_num_per_class must be positive.')

    seed = self.config.class_balanced_seed
    num_classes = self.config.num_classes
    class_datasets = []
    for class_id in range(num_classes):
      class_ds = ds.filter(
          lambda x, class_id=class_id: tf.equal(x.label, class_id))
      if seed is not None:
        class_ds = class_ds.shuffle(
            buffer_size=max(1000, 10 * num_per_class),
            seed=seed + class_id,
            reshuffle_each_iteration=False,
        )
      class_ds = class_ds.take(num_per_class)
      class_datasets.append(class_ds)

    balanced_ds = class_datasets[0]
    for class_ds in class_datasets[1:]:
      balanced_ds = balanced_ds.concatenate(class_ds)
    return balanced_ds
  ...
  def load_dataset(
    ...
    if (not is_training and
        self.config.class_balanced_num_per_class is not None):
      ds = self._apply_class_balanced_subset(ds)
    if shard_data:
