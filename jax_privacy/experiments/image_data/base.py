...
class DatasetConfig(metaclass=abc.ABCMeta):
  ...
  Attributes:
    ...
    class_balanced_num_per_class: Optional per-class cap for a fixed,
      class-balanced subset (evaluation only).
    class_balanced_seed: Optional seed for deterministic per-class shuffling.
  """
  ...
  name: str
  split_content: str
  class_balanced_num_per_class: int | None = None
  class_balanced_seed: int | None = None
