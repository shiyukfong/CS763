...
@dataclasses.dataclass(kw_only=True, slots=True)
class NCEvaluationConfig:
  """Configuration for neural collapse evaluation.

  Attributes:
    batch_size: Batch-size for NC evaluation.
    max_num_batches: Maximum number of batches to use in an NC run.
    params_to_eval: Parameter sets to evaluate (e.g. ('last', 'ema')).
    use_single_device: Whether to run NC on a single device.
    host0_only: Whether to compute NC only on host 0.
  """

  batch_size: int
  max_num_batches: int | None = None
  params_to_eval: tuple[str, ...] = ('last',)
  use_single_device: bool = True
